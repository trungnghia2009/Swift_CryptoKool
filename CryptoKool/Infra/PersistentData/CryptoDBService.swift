...
final class CryptoDBService: CoreDataInterface {
    ...
    func fetchData() -> AnyPublisher<[CryptoDB], Error> {
        return Future<[CryptoDB], Error> { [weak self] promise in
            ...
            do {
                ...
                CKLogger.info("Could not fetch \(error), \(error.userInfo)")
